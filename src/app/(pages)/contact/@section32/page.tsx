  ...
  return (
    ...
    >
      <div className={`w-full text-center mb-4`}>
        <h1 className={`h1 md:text-5xl text-3xl`}>Need more help?</h1>
        <p className={`pp md:text-lg text-sm`}>
          Explore our various support channels below for specific inquiries,
          whether you
          <br />
          need technical assistance, partnership info, or billing support
        </p>
      </div>
      <div className={`flex gap-8 lg:flex-row flex-col`}>
        {helpInfos.map((infos) => (
          ...
          >
            ...
            />
            <div>
              <h4 className={`h1 text-2xl mb-3`}>{infos.title}</h4>
              <p className={`pp mb-6`}>{infos.text}</p>
              <div
